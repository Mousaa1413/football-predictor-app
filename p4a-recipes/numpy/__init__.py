"""Local numpy recipe: p4a master + fix missing <unordered_map> on NDK libc++.

Upstream p4a pins numpy v2.3.0 which uses std::unordered_map in
unique.cpp without including <unordered_map>. That compiles on some
host libcs via transitive includes, but fails with Android NDK r25b
libc++ (clang):

  unique.cpp:123:6: error: no template named 'unordered_map' in namespace 'std'
"""
...
class NumpyRecipe(MesonRecipe):
    ...
    patches = ["add_unordered_map_include.patch"]
